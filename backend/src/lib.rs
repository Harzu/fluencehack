...
fn do_request(req: String) -> AppResult<Value> {
    ...
    match request {
        Request::CreateBattle {
            ...
            room_id,
            army
        } => GAME_MANAGER.with(|gm| gm.borrow_mut().create_game(player_name, room_id, army)),
        Request::GetRooms {
            count
        } => GAME_MANAGER.with(|gm| gm.borrow().get_rooms(count)),
        Request::Connect {
            player_name,
            room_id,
            army
        } => GAME_MANAGER.with(|gm| gm.borrow_mut().connect(player_name, room_id, army)),
        Request::GetUsers {
            room_id
        } => GAME_MANAGER.with(|gm| gm.borrow_mut().get_users(room_id)),
    }
}
